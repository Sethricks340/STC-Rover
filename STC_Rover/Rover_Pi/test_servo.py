    ...
    try:
        async for message in websocket:
            if isinstance(message, bytes) and len(message) == 5:
                ...
                last_command_time = time.time()
                print(
                    f"Opcode: {opcode}, Motor: {motor_number}, Power: {power}, PWM: {pwm}, Dir: {direction}"
                )
                # motors
                if opcode == 0:
                    if pwm < 50:
                        ...
                        if not motor_number:
                            GPIO.output(
                                IN1, GPIO.LOW if direction else GPIO.HIGH
                            )
                            GPIO.output(
                                IN2, GPIO.HIGH if direction else GPIO.LOW
                            )
                            pwmA.ChangeDutyCycle(duty)
                        else:
                            GPIO.output(
                                IN3, GPIO.HIGH if direction else GPIO.LOW
                            )
                            GPIO.output(
                                IN4, GPIO.LOW if direction else GPIO.HIGH
                            )
                            pwmB.ChangeDutyCycle(duty)
                
                # servo
                elif opcode == 1:
                    # pwm 0–255 → servo duty
                    duty = 2.5 + (pwm / 255) * 10

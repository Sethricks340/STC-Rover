...
try:
    # Tailscale rover IP:
    ip_string = "ws://100.94.206.108:8081/motors" # TODO: change to port
    ws.connect(ip_string)
    print("Controls WebSocket connected")
    ws_connected = True
except Exception as e:
        ...
        while True:
            try:
                ...
                async with websockets.connect(uri) as websocket:
                    ...
                    while True:
                        data = await websocket.recv()
                        if data.startswith("MIC:"): # TODO: remember to change to MIC in windows code
                            audio_bytes = base64.b64decode(data[4:])
                            audio_array = np.frombuffer(audio_bytes, dtype=np.float32)
